...
/* ============================================================================
   ...
   Jeden dokument ciągły, nie plik na zestaw: ten spis czyta się na ekranie
   z góry na dół, a nie rozcina na kartki. Zestaw w nim to jedna karta —
   zdjęcie z lewej, treść z prawej. Układ pionowy zostawiał pół strony
   powietrza pod krótkim opisem.

   Ceny TGTG NIE WCHODZĄ do żadnego rachunku. Food cost, marża i rabat liczą
   się z kanałów Vending i Dostawa i mają się nie zmienić ani o grosz przez to,
   że zestaw dostał drugą cenę na inny serwis. Gdyby kiedyś miało być inaczej,
   to jest decyzja właściciela, nie sprzątanie w kodzie.
   ========================================================================== */
function pdfTgg(sety){
  const lista = (sety || []).filter(Boolean);
  if(!lista.length){ alert('Nie ma czego drukować — lista zestawów jest pusta.'); return; }
  // Brak kwoty nie może być pustym miejscem: ten dokument idzie na zewnątrz
  // i dziura w nim wygląda jak zero, a nie jak „jeszcze nie ustalone".
  const kwota = (lab, v) => `<div class="kwota"><div class="lab">${lab}</div>
    <div class="v${v==null?' brak':''}">${v==null ? '— uzupełnij' : zl(v)}</div></div>`;
  const tresc = lista.map(st=>{
    const c = CALC.setCalc(st);
    return `<section class="tkar">
      ${st.photo ? `<img class="fot" src="${st.photo}" alt="${esc(st.name)}">`
                 : '<div class="fot brak">bez zdjęcia</div>'}
      <div class="tre">
        <h2>${esc(st.name)}</h2>
        <div class="pod">${c.pieces} ${mnoga(c.pieces,'kawałek','kawałki','kawałków')}</div>
        <div class="kwoty">${kwota('Cena zestawu', st.tggCena)}${kwota('Cena opakowania', st.tggOpak)}</div>
        ${st.opis ? `<div class="opis">${esc(st.opis)}</div>` : ''}
      </div></section>`;
  }).join('');
  zrobPdf(dokumentDruku('Zestawy — Too Good To Go', TGG_STYL, tresc, 11),
          'zestawy-toogoodtogo', stopkaDruku('Zestawy — Too Good To Go'));
}
/** Karta nie łamie się między stronami. Zdjęcie na dole jednej, a cena na górze
    następnej to najgorsze, co może zrobić dokument oglądany z telefonu. */
const TGG_STYL = `
  .tkar{display:flex;gap:6mm;padding:5mm 0;border-top:1px solid #DDD9D2;
        break-inside:avoid;page-break-inside:avoid}
  .tkar:first-of-type{border-top:0;padding-top:1mm}
  .tkar .fot{width:46mm;height:34mm;flex:none;border-radius:5px;object-fit:cover;
        background:#EFEDE8}
  .tkar .fot.brak{display:flex;align-items:center;justify-content:center;
        color:#8A8781;font-size:.78em}
  .tkar .tre{flex:1;min-width:0}
  .tkar h2{font-size:1.32em;margin:0;font-weight:600;letter-spacing:-.01em;line-height:1.2}
  .tkar .pod{color:#8A8781;font-size:.84em;margin:.1em 0 .55em}
  .tkar .kwoty{display:flex;gap:9mm;margin-bottom:.5em}
  .tkar .kwota .lab{font-size:.66em;font-weight:700;letter-spacing:.13em;
        text-transform:uppercase;color:#8A8781}
  .tkar .kwota .v{font-size:1.18em;font-weight:600;letter-spacing:-.01em}
  .tkar .kwota .v.brak{color:#B26B00;font-weight:500}
  .tkar .opis{font-size:.95em;line-height:1.5;color:#333;white-space:pre-wrap}`;
...
function vSets(){
  ...
  after('sets',()=>{
    ...
    document.querySelector('[data-act="tggSets"]').addEventListener('click',
  ...
  if(sel){
    ...
    detail = podgladPozycji({
      ...
      dodatkowe:[
        ...
        // Poza rachunkiem, więc na końcu i na szaro — żeby nikt nie szukał ich
        // w food coście, którego nie ruszają.
        {l:'Too Good To Go — cena zestawu', v: sel.tggCena!=null?zl(sel.tggCena):'—',
         mut: sel.tggCena==null},
        {l:'Too Good To Go — cena opakowania', v: sel.tggOpak!=null?zl(sel.tggOpak):'—',
         mut: sel.tggOpak==null},
      ],
  ...
  return paskListy({tytul:'Zestawy', licznik:`${active(DB.sets).length} aktywnych`,
    ...
    akcje:'<button class="btn" data-act="pdfSets" title="Skład wszystkich zestawów na kartkę">⎙ PDF</button>'
      ...
      + '<button class="btn" data-act="tggSets" title="Karta zestawów dla Too Good To Go — '
      + 'jeden dokument ciągły ze zdjęciem, ceną i opisem">⎙ TGTG</button>',
...
function editSet(id){
  ...
  openDlg(id?'Edytuj zestaw':'Nowy zestaw',`
    ...
    <h3 style="margin:16px 0 8px">Too Good To Go</h3>
    <div class="hint" style="margin-bottom:8px">Dwie kwoty do karty zestawów dla TGTG.
      <b>Nie wchodzą do food costu ani do marży</b> — kanały Vending i Dostawa liczą się
      dokładnie tak jak dotąd. Puste pola znaczą „jeszcze nie ustalone" i tak też wyjdą
      na karcie, żeby brak było widać.</div>
    <div class="grid" style="grid-template-columns:1fr 1fr">
      <div><label class="f">Cena zestawu (brutto)</label>
        <input id="sTggCena" type="number" step="any" min="0" value="${st.tggCena??''}"></div>
      <div><label class="f">Cena opakowania</label>
        <input id="sTggOpak" type="number" step="any" min="0" value="${st.tggOpak??''}"></div>
    </div>
    ...
    [ {label:'Anuluj'},
      {label:'Zapisz',cls:'pri',fn:()=>{
        ...
        st.tggCena=numOrNull('sTggCena');
        st.tggOpak=numOrNull('sTggOpak');
        recalc();
